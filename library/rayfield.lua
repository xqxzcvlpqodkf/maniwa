...
function RayfieldLibrary:CreateWindow(Settings)
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateCanvas(CanvasSettings)
            CanvasSettings = CanvasSettings or {}

            local canvas = nil
            
            if CanvasSettings.Scrollable then
                canvas = Instance.new("ScrollingFrame")
                canvas.CanvasSize = CanvasSettings.CanvasSize or UDim2.new(0, 0, 0, 0)
                canvas.ScrollBarThickness = 6
                canvas.ScrollBarImageColor3 = SelectedTheme.TextColor
            else
                canvas = Instance.new("Frame")
            end

            canvas.Name = CanvasSettings.Name or "Canvas"
            canvas.Size = CanvasSettings.Size or UDim2.new(1, -10, 0, 200)
            canvas.Position = CanvasSettings.Position or UDim2.new(0, 5, 0, 0)
            canvas.BackgroundColor3 = CanvasSettings.BackgroundColor or SelectedTheme.ElementBackground
            canvas.BackgroundTransparency = CanvasSettings.BackgroundTransparency or 0
            canvas.ClipsDescendants = (CanvasSettings.ClipsDescendants ~= false)
            canvas.BorderSizePixel = 0
            canvas.Parent = TabPage
